...
///
/// Defined in `mindleak-model` (ADR-0140 decision 5) so it can be shared with
/// Ackplane's recall discrimination logic without pulling all of
/// `mindleak-core` across the federation boundary (ADR-0082); re-exported
/// here so existing call sites in this crate are unaffected.
pub use mindleak_model::NodeType;
