...
pub use mindleak_model::ModelHealth;
...
impl Consolidator {
    ...
    /// Probe the optional model only when a caller explicitly asks for it.
    pub fn model_health(&self) -> ModelHealth {
        ...
        match self.consolidate(&["Model health probe.".to_string()]) {
        ...
        let content = value
            ...
            .ok_or_else(|| {
                model_error(
                    ...
                    true,
                    "chat completion missing choices[0].message.content",
                )
            })?;
...
mod tests {
    // Generated by AI (UnitTest MCP)
    use super::*;
    use mindleak_model::test_support::json_endpoint;

    #[test]
    fn debug_redacts_the_api_key() {
        const SECRET: &str = "sk-mindleak-super-secret-token";
        let mut configured = Consolidator::new("http://localhost:11434/v1", "glm4:9b");
        ...
        assert!(
            format!("{:?}", Consolidator::new("http://h", "m")).contains("<unset>"),
    ...
    fn model_health_distinguishes_not_configured_from_unreachable() {
        let not_configured = Consolidator::new("", "").model_health();
        assert!(!not_configured.configured);
        assert_eq!(not_configured.failure_reason, None);

        let unreachable = Consolidator::new("http://127.0.0.1:1/v1", "test").model_health();
    ...
    fn model_health_reports_a_reachable_non_json_endpoint() {
        ...
        let health = Consolidator::new(base_url, "test").model_health();
        server.join().unwrap();
