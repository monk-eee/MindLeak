...
mod tests {
    ...
    fn embedder_returning(body: &str) -> (Embedder, thread::JoinHandle<()>) {
    ...
    fn embed_batch_rejects_non_numeric_components() {
        let (embedder, server) =
            embedder_returning(r#"{"data":[{"index":0,"embedding":[0.1,"bad",0.2]}]}"#);

        let error = embedder.embed_batch(&["hello".to_string()]).unwrap_err();
        server.join().unwrap();
    ...
    fn embed_batch_rejects_components_that_are_not_finite_f32() {
        let (embedder, server) =
            embedder_returning(r#"{"data":[{"index":0,"embedding":[3.5e38,0.2]}]}"#);
    ...
    fn embed_batch_rejects_inconsistent_dimensions() {
        let (embedder, server) = embedder_returning(
            r#"{"data":[{"index":0,"embedding":[0.1,0.2]},{"index":1,"embedding":[0.1,0.2,0.3]}]}"#,
        );
