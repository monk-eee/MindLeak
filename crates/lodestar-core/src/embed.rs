...
mod tests {
    ...
    fn stub_embedder(body: &'static str) -> (Embedder, std::thread::JoinHandle<()>) {
        let (base_url, server) = json_endpoint("/v1", body.as_bytes().to_vec());
        (
            Embedder {
                base_url,
    ...
    #[test]
    fn a_complete_batch_request_returns_vectors_in_input_order() {
        // The old server replied after one partial read, so unread request bytes
        // intermittently reset the Windows client (os error 10054). The shared
        // fixture consumes the complete declared body before replying.
        ...
        let (embedder, server) = stub_embedder(
            r#"{"data":[{"index":1,"embedding":[0.0,1.0]},{"index":0,"embedding":[1.0,0.0]}]}"#,
        );

        let vectors = embedder.embed_batch(&["first".to_string(), "second".to_string()]);
        server.join().unwrap();

        assert_eq!(vectors.unwrap(), vec![vec![1.0, 0.0], vec![0.0, 1.0]]);
    }
    ...
    fn a_malformed_response_is_refused_rather_than_quietly_reshaped() {
        ...
        for (body, input) in cases.iter().zip(inputs) {
            let (embedder, server) = stub_embedder(body);
            let result = embedder.embed_batch(&input);
            server.join().unwrap();
            assert!(result.is_err(), "should have refused: {body}");
        }
    }
