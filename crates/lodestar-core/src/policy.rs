...
pub fn fleet_delivery_pack() -> ConstitutionPack {
    let clauses = vec![
        ...
        PackClause {
            ...
            title: "One publishing owner per task branch".to_string(),
            statement:
                "One owner publishes each task branch from its clean checkout or linked worktree, pushing the current branch's exact HEAD to the same remote branch."
                    .to_string(),
            rationale:
                "Concurrent publishers can reconcile the same divergence differently. Branch ownership keeps publication a decision without making one physical checkout a global bottleneck."
                    .to_string(),
            default_scope: Some("workflow:git.publish".to_string()),
            evidence_contract: Some(
        ...
        PackClause {
            key: "fleet.isolated_worktree".to_string(),
            kind: GoalKind::Constraint,
            title: "Concurrent workstreams use isolated worktrees".to_string(),
            statement:
                "Each concurrent agent workstream uses its own branch and linked worktree. Sharing one writable checkout is a reviewed exception, not the default."
                    .to_string(),
            rationale:
                "Separate worktrees isolate files, indexes, and branch selection while Lodestar coordinates ownership and MindLeak shares repository learning."
                    .to_string(),
            default_scope: Some("workflow:topology".to_string()),
            evidence_contract: Some(
                "The active sessions' declared branches and worktree identities for one repository id."
                    .to_string(),
            ),
            default_consequence: Some(Consequence::Review),
            suggested_controls: vec!["declared session topology comparison".to_string()],
        },
        PackClause {
        ...
        PackClause {
            ...
            statement:
                "An agent commits only paths within the scope it declared on its claim; the staged set may not escape that scope."
                    .to_string(),
            rationale:
                "Worktree isolation prevents accidental staging across agents, but an out-of-scope commit still misattributes work and weakens conformance evidence."
                    .to_string(),
...
mod tests {
    ...
    #[test]
    fn fleet_delivery_v2_requires_isolated_worktrees_without_privileging_one_checkout() {
        ...
        let isolated = pack
            .clauses
            .iter()
            .find(|clause| clause.key == "fleet.isolated_worktree")
            .unwrap();
        assert_eq!(isolated.default_scope.as_deref(), Some("workflow:topology"));
        assert_eq!(isolated.default_consequence, Some(Consequence::Review));

        let publisher = pack
            .clauses
            .iter()
            .find(|clause| clause.key == "fleet.single_publisher")
            .unwrap();
        assert!(publisher
            .statement
            .contains("clean checkout or linked worktree"));
        assert!(!publisher.statement.contains("primary checkout"));
    }
}
