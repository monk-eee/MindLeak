...
//! (`ackplane_protocol::context_packet`): immutable packets and attributed,
//! idempotent use receipts (decisions 1, 5, 7). `context_service` composes
//! this store with the deterministic compiler and authenticated NodeSync transport.
        ...
        let row = transaction
            .query_opt(
                "SELECT task_id, goal_id, agent_session_id FROM context_packets \
                 WHERE tenant_id = $1 AND repository_id = $2 AND packet_id = $3 FOR UPDATE",
            ...
            .execute(
                "INSERT INTO context_packet_use_receipts \
                 ...
                 SELECT $1, $2, $3, $4, $5, $6 WHERE NOT EXISTS ( \
                     SELECT 1 FROM context_packet_use_receipts \
                     WHERE tenant_id = $1 AND repository_id = $2 AND packet_id = $3 \
                       AND status = $4 AND reason IS NOT DISTINCT FROM $5 AND occurred_at = $6)",
                &[
                    tenant_id,
                    repository_id,
                    &receipt.packet_id,
