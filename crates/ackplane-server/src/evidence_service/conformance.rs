...
pub(super) fn conformance_store_error(error: ConformanceStoreError) -> Status {
    match error {
        ...
        ConformanceStoreError::TooManyFindingCodes => {
            Status::invalid_argument("finding_codes exceeds the bounded finding vocabulary")
        }
        ConformanceStoreError::DuplicateFindingCode => {
            Status::invalid_argument("finding_codes must not repeat a code")
        }
        ConformanceStoreError::FindingCodeCountExceedsFindingCount => {
            Status::invalid_argument("finding_codes cannot exceed finding_count")
        }
        ...
        ConformanceStoreError::UnknownStoredFindingCode(code) => {
...
fn conformance_finding_codes(raw: &[i32]) -> Result<Vec<ConformanceFindingCode>, Status> {
    raw.iter()
        .map(|code| {
            ConformanceFindingCode::from_i32(*code)
                .ok_or_else(|| Status::invalid_argument("conformance finding code is invalid"))
        })
        .collect()
}
...
fn proto_finding_code(code: ConformanceFindingCode) -> i32 {
    match code {
        ConformanceFindingCode::EvidenceMissing => {
            v1::ConformanceFindingCode::EvidenceMissing as i32
        }
        ConformanceFindingCode::EvidenceStale => v1::ConformanceFindingCode::EvidenceStale as i32,
        ConformanceFindingCode::ScopeUncovered => v1::ConformanceFindingCode::ScopeUncovered as i32,
        ConformanceFindingCode::PolicyDrift => v1::ConformanceFindingCode::PolicyDrift as i32,
        ConformanceFindingCode::PolicyViolation => {
            v1::ConformanceFindingCode::PolicyViolation as i32
        }
        ConformanceFindingCode::ReceiptMissing => v1::ConformanceFindingCode::ReceiptMissing as i32,
        ConformanceFindingCode::HumanReviewRequired => {
            v1::ConformanceFindingCode::HumanReviewRequired as i32
        }
    }
}
...
impl EvidenceGrpcService {
    pub(super) async fn record_conformance_impl(
        ...
        let finding_codes = conformance_finding_codes(&request.finding_codes)?;
            ...
            .map_err(conformance_store_error)?
        {
            let matches = existing.task_id == conformance_request.task_id
                ...
                && existing.finding_codes == conformance_request.finding_codes
...
mod tests {
    ...
    #[test]
    fn finding_codes_map_between_the_wire_and_closed_storage_vocabulary() {
        let codes = conformance_finding_codes(&[
            v1::ConformanceFindingCode::PolicyDrift as i32,
            v1::ConformanceFindingCode::HumanReviewRequired as i32,
        ])
        .expect("known finding codes must map into storage values");
        ...
        assert_eq!(
            proto_finding_code(ConformanceFindingCode::PolicyDrift),
            v1::ConformanceFindingCode::PolicyDrift as i32
        );
        let error = conformance_finding_codes(&[99]).expect_err("unknown code must be refused");
        assert_eq!(error.code(), tonic::Code::InvalidArgument);
        assert_eq!(error.message(), "conformance finding code is invalid");
    }
}
