...
impl v1::claim_delegation_service_server::ClaimDelegationService for ClaimDelegationService {
    ...
    async fn park_claim(
        &self,
        request: Request<v1::ClaimParkRequest>,
    ) -> Result<Response<v1::ClaimParkResult>, Status> {
        let request = request.into_inner();
        let tenant_id =
            required(request.tenant_id, "tenant_id").map_err(Status::invalid_argument)?;
        let repository_id =
        ...
        let parked = self
            ...
            .await
            .map_err(map_store_error)?;
        Ok(Response::new(v1::ClaimParkResult {
            parked,
            diagnostic: String::new(),
        }))
    }

    async fn answer_claim(
        &self,
        request: Request<v1::ClaimAnswerRequest>,
    ) -> Result<Response<v1::ClaimLeaseResult>, Status> {
        let request = request.into_inner();
        let tenant_id =
            required(request.tenant_id, "tenant_id").map_err(Status::invalid_argument)?;
...
mod wire;
use wire::{active_claim_to_wire, map_store_error, request_from_wire, required, result_to_wire};
