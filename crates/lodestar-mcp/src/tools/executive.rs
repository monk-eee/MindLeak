...
use super::conformance::{delivered_completion_conformance, parse_evidence};
use super::{
...
pub(super) fn definitions() -> Vec<Value> {
    vec![
        json!({
            "name": "task_create",
            "description": "Create work serving a goal. With `title`: one task. `blocked_by` keeps it unclaimable until that predecessor completes aligned. `also_serves` declares additional goals this work serves, fixed for the task's life. Without `title`: decomposes the goal into claimable tasks instead (model-backed when reachable, single-task fallback otherwise). Never refuses for existing work: `duplicates` names a live exact-title match under the goal; `prior_work` lists every task ever created under it.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "goal_id": { "type": "string" },
