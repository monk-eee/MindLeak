...
pub(in crate::tools) fn definitions() -> Vec<Value> {
    vec![
        ...
        json!({
            "name": "task_claim",
            "description": "Ownership and the lease, chosen by `step`. `claim`: atomic claim + lease with optional advisory `paths`/MindLeak `symbols`; `won=true` only if this agent won, and a loss names why. Succeeds for ANY agent once the task is `open` or `claimed` with an expired lease — no `recover` step needed to take over a stranded claim. Omitting `paths`/`symbols` on re-claim preserves that part of the existing scope; an explicit array (including `[]`) replaces or clears it. A won claim resolves active clauses against those paths (`scope_advice`; `review` names an uncovered goal to add via `also_serves`), reports `title_twin` naming another live task under this same goal sharing this exact title if one exists (ADR-0099), and returns the evidence window completion later validates against. `also_serves` can extend a held claim until conformance has judged it — refused after. `renew`: extends a still-live lease owned by this agent; after expiry only a fresh `claim` opens a new window. `reconnect_clause` (renew only, ADR-0109): after a successful renewal, ask to move this task's own goal_id from a superseded clause onto its active same-slug successor — refused (with a distinct reason) unless exactly one such successor exists. `release`: hands the claim back to open, owner-guarded. `recover`: only for two cases `claim` cannot reach — a pre-ADR-0054 legacy owner string (rare today), or an early `paused`-task transfer with a named human reviewer (`expected_owner` and `reason` required; `human` is attributed, not authenticated, and must differ from both owners). Not the general path for an ordinary lapsed lease; use `claim` for that.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "task_id": { "type": "string" },
        ...
        json!({
            ...
            "inputSchema": {
                ...
                "properties": {
                    ...
                    "include_terminal": { "type": "boolean", "default": true, "description": "board: done/abandoned tasks; false is live-only" },
                    "detail": { "type": "boolean", "default": true, "description": "false omits scope/claim_window/receipt/acceptance" },
