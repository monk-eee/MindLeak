...
fn supervisor(state: &Path, workspace: &Path, node_directory: &Path) -> Command {
...
fn listener(directory: &Path) -> Listener {
    ListenerOptions::new()
        .name(ackplane_client::companion::endpoint_name(directory).unwrap())
        .create_tokio()
        .unwrap()
}
...
async fn a_second_idle_supervisor_cannot_share_the_state_directory() {
    ...
    let mut first = supervisor(&state, root.path(), root.path())
        .spawn()
        .unwrap();
    ...
    let second = supervisor(&state, root.path(), root.path())
        .spawn()
        .unwrap();
...
async fn killing_an_idle_owner_releases_the_directory_for_restart() {
    ...
    let mut restarted = supervisor(&state, root.path(), root.path())
        .spawn()
        .unwrap();
