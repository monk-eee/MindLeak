...
use super::{opt_i64, opt_str, refused, rendered_result, req_str, required_i64, text_result};
...
pub(super) fn dispatch(
    ...
    match name {
        ...
        "reset_database" => Some((|| {
            ...
            let outcome = engine.reset_database(&confirmation).map_err(|e| {
                if matches!(e, MindLeakError::InvalidArgument(_)) {
                    refused(e)
                } else {
                    e.to_string()
                }
...
mod tests {
    ...
    fn lifecycle_tools_export_backup_and_confirm_reset() {
        ...
        // Regression: the client-visible error must never contain the
        // telemetry-only refused marker, and the refusal must be recorded as
        // `refused`, not `error` -- a working guard is not a broken tool. See
        // gaps.d/a-refused-call-is-counted-as-a-failed-call.md.
        assert!(!rejected.contains('\u{1}'));
