...
| `stalled_work` | Every task that is not progressing and the fact that stalled it: lapsed leases, work awaiting a human, blocks behind something no agent will advance, blocks naming a task that is not on the board, and parked work. Read-only, evidence-free, and deliberately threshold-free — it reports how long a stall has been true and leaves "is that too long?" to a person. |
