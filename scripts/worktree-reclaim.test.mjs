...
// Git's lock was lost before reclamation, allowing artifact deletion before Git refused removal.
test("a Git lock preserves an otherwise reclaimable worktree", () => {
  const verdict = classifyWorktree({ ...idle, locked: true });
  assert.equal(verdict.reclaim, false);
  assert.match(verdict.reason, /Git worktree is locked/);
});

test("reads Git locks with and without reasons from real worktrees", (context) => {
  const root = mkdtempSync(join(tmpdir(), "reclaim-lock-"));
  context.after(() => rmSync(root, { recursive: true, force: true }));
  const anchor = join(root, "repo");
  const retained = join(root, "retained");
  mkdirSync(anchor);
  const git = (args) => {
    const result = isolatedGit(args, anchor);
  ...
  git([
    "-c",
    "user.name=Reclaim Test",
    "-c",
    "user.email=reclaim@example.invalid",
    "-c",
    "core.hooksPath=",
    "commit",
    "--quiet",
  ...
  git(["worktree", "add", "--quiet", "-b", "retained", retained]);
  const record = () =>
    readWorktrees(anchor).find((worktree) => worktree.branch === "retained");
  assert.equal(record().locked, false);
  git(["worktree", "lock", retained]);
  assert.equal(record().locked, true);
  git(["worktree", "unlock", retained]);
  git([
    "worktree",
    "lock",
    "--reason",
...
// A lock acquired after the preview must stop artifact deletion, not only Git's later removal.
test("a Git lock acquired after the preview prevents destructive calls", () => {
  const operations = [];
  const verdict = revalidateBeforeReclaim(idle, {
    readWorktree: () => ({ ...idle, locked: true }),
    readClaimState: () => ({ available: true, branches: new Set() }),
  });
  if (verdict.reclaim) {
    reclaimEntry(
      { worktree: idle, artifacts: [{ dir: `${idle.path}/target/debug` }] },
      {
        anchor: "/repo",
        rm: (path) => {
...
test("unreadable worktree facts cannot authorize reclamation", () => {
  const verdict = revalidateBeforeReclaim(idle, {
    readWorktree: () => null,
    readClaimState: () => ({ available: true, branches: new Set() }),
  });
  assert.equal(verdict.reclaim, false);
  assert.match(verdict.reason, /worktree facts could not be re-read/);
});

test("fresh unlocked facts preserve normal reclamation", () => {
  const verdict = revalidateBeforeReclaim(idle, {
    readWorktree: (worktree) => ({ ...worktree, locked: false }),
    readClaimState: () => ({ available: true, branches: new Set() }),
  });
  assert.equal(verdict.reclaim, true);
});

test("a changed worktree identity cannot authorize the old deletion plan", () => {
  for (const changed of [{ path: "/elsewhere" }, { branch: "fix/new-work" }]) {
    const verdict = revalidateBeforeReclaim(idle, {
      readWorktree: () => ({ ...idle, ...changed }),
      readClaimState: () => ({ available: true, branches: new Set() }),
    });
    assert.equal(verdict.reclaim, false);
    assert.match(verdict.reason, /worktree identity changed/);
  }
});
...
// Artifact-only sweeping must honor the same explicit retention lock as whole-tree reclamation.
test("a Git lock preserves an otherwise sweepable artifact", () => {
  const verdict = classifyArtefact(cache({ locked: true }), { now: NOW });
