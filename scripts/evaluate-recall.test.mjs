...
test("relevance requires a whole query anchor in the returned node text", () => {
  const hits = [
    {
      label: "fix(delivery): arming auto-merge is not a deadline",
      content: null,
    },
    { label: "Routine branch reconciliation", content: null },
  ];

  assert.deepEqual(relevantHits(hits, ["armed", "merge"]), [hits[0]]);
});

test("unrelated non-empty results do not count as a successful recall", () => {
  const hits = [
    { label: "docs: record task evidence", content: "claim metadata" },
  ];

  assert.deepEqual(relevantHits(hits, ["powershell", "succeeded"]), []);
});
