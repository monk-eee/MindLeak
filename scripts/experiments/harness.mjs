...
export function driveServer(exe, root = process.cwd(), extraEnv = {}) {
  ...
  const env = { ...process.env, MINDLEAK_DB: path.join(dir, "graph.db"), ...extraEnv };
...
export async function embed(text, opts = {}) {
  const base = (opts.url ?? process.env.MINDLEAK_EMBED_URL ?? "http://localhost:11434/v1").replace(
    /\/+$/,
    ""
  );
  const model = opts.model ?? process.env.MINDLEAK_EMBED_MODEL ?? "nomic-embed-text";
  ...
  return (queryText, { exclude } = {}) => {
    ...
    return ids
      .map((id, i) => {
        ...
        let dot = 0;
        for (const [term, weight] of qv) dot += weight * (vectors[i].get(term) ?? 0);
