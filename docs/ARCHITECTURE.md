...
completed attempts also resolve lost responses after removal. `cli.rs` exposes
explicit inspect/confirm commands with a digest and reason. No recovery path
spawns, signals, announces a new session, renews authority or completes Work.
Live, unproven, old or contradictory runs remain refused. The
[operator walkthrough](../crates/ackplane-supervisor/README.md#recover-a-stopped-run)
states bounds, retry semantics and remaining limitations.
...
`WorkCommandService` (`work_command_store/service.rs`) is the ledger's only production writer. It evaluates an authentication result before the first command lookup: `LoopbackDevelopment` returns `authorization_unavailable` (still `submit`/`confirm`'s response for a `MissingPrincipal` authentication result, and for any deployment that has not adopted ADR-0128/ADR-0142's verified-principal recognition), while missing, forged, tenant/repository-out-of-scope, command, policy, or delegation mismatches against a `Verified` principal return typed refusals without disclosing a target. A verified in-scope request records only the immutable command and a `pending_confirmation` receipt; `confirm` re-checks identity and the exact payload digest before executing. `refusal_for`'s policy check compares `principal.policy_refs` against the request's by plain equality (a bug fix landed alongside ADR-0142: it used to also refuse unconditionally whenever a principal's `policy_refs` was empty, which would have permanently blocked ADR-0142's own no-adopted-policy principal from submitting anything).
...
paths. ADR-0128 recognized the hardened loopback token as a real verified
principal for a self-hosted, single-tenant deployment (Administration's own
adopted policy still gates its four privileged classes); ADR-0142 extends
that same recognition to Work commands (a verified principal, the full
closed command vocabulary, no adopted policy -- see decision 5's reasoning)
and to Design/Constitution mutations (the verified principal is now the
authoritative `proposed_by`/`actor`/`author`, never a caller-supplied
string). Enrolment and normal node-signed claim lifecycle
(`delegate`/`renew`/`release`) remain behind Ackplane's typed gRPC service
boundaries; the Bridge never reaches them. Current routes, each 404 on a
repository the tenant has not enrolled rather than leaking a distinguishable
error:
...
| `GET /work` | The task board and explicit Work command workflow: published-goal/scoped-task creation, advertised agent selection, questions, and review submission, with separate immutable preview and confirmation. Publication diagnostics retain `current`, `claims_only`, and `not_published`; claims are never invented into tasks. Declared artifact and fully qualified symbol scope links into `GET /graph`. |
| `GET /static/work-page.mjs`, `GET /static/work-command-client.mjs`, `GET /static/work-page.css` | Embedded, read-only Work page modules and styles with fixed content types; these routes never read tenant data or arbitrary filesystem paths. |
...
| `POST /api/v1/repositories/:repository_id/work/commands` | ADR-0125: submits one of the ten closed Work commands (`CreateWork`/`RouteWork`/`ReleaseLease`/`AnswerWait`/`SubmitReview`/`Assign`/`Steer`/`Pause`/`Resume`/`Drain`, tagged by `kind` in one JSON envelope) through `WorkCommandService` -- never `WorkStore`/`ClaimStore` directly. Under the Bridge's hardened loopback profile, now a real verified principal for a self-hosted, single-tenant deployment (ADR-0142), a well-formed and correctly-attributed request records a genuine `pending_confirmation` preview rather than the permanent `authorization_unavailable` refusal ADR-0125 shipped before this ADR. |
| `POST /api/v1/repositories/:repository_id/work/commands/:command_id/confirm` | Confirms a previously submitted command against its exact payload digest -- the same identity/tenant/repository re-check ADR-0125 decision 8 specified, now against the ADR-0142 verified principal. |
| `GET /design` | The Design Board page (static HTML/JS). |
| `GET`/`POST /api/v1/repositories/:repository_id/designs` | Lists designs, or proposes a new one (`DesignStore::register`) -- a digest-checked idempotent insert of an opaque design record with bounded title/summary/source_version and a closed lifecycle state. `proposed_by` is always the Bridge's verified principal (ADR-0142); an optional caller-supplied label is accepted but never persisted as identity. |
| `GET /api/v1/repositories/:repository_id/designs/:design_id` | One design's detail. |
| `POST /api/v1/repositories/:repository_id/designs/:design_id/decisions` | Appends a lifecycle transition (`DesignStore::record_decision`) to a design's append-only decision history; `actor` is always the Bridge's verified principal (ADR-0142), not a caller-supplied string -- ADR-0121 decision 3's broader legal-transition *policy* (which transitions are permitted) remains deferred to a future typed command. |
