...
The Bridge Work read response still reports ADR-0125's closed Work-command vocabulary as unconditionally `authorization_unavailable` (`work_api.rs`'s `command_capabilities()`) and the Work page still renders every named control disabled with that reason -- this static, read-only capability list was not updated by ADR-0142 and is now stale relative to what the command route above actually does; narrowing that drift is a filed follow-up (`gaps.d/`), not silently left undocumented.
