...
It does not open a network listener or feed a directive into the inbox itself. A future NodeSync transport adapter reads pending outbound frames, acknowledges accepted delivery, and delivers received directives to it. This crate establishes the local durable queue boundary that adapter must preserve.
