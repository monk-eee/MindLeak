...
| `GET /work` | The bounded Industrial Work page. It labels each repository's projection as `current`, `claims_only`, or `not_published`, so an empty native Work list never masquerades as a clean task board. Claims-only rows remain lease records with their Ackplane owner, branch, expiry, and declared scope; Bridge never invents Local task title, acceptance, or lifecycle data. |
