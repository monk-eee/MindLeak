...
| `retire_control` | Stand a control down when it is superseded or was registered under the wrong id. Retirement is not deletion: the control keeps recording what it enforced, so observations naming it resolve as `unknown` rather than disappearing. Without it a misregistered control is permanent, because its version can never move backwards. |
...
| `list_designs` | The durable design ledger, optionally filtered by status. Retired records (ADR-0042) are omitted unless `include_retired` — the board shows live decisions while the audit trail stays complete. |
...
| `reopen_undecided_design` | Return a design whose status nobody ever decided to `proposed`, so a person can decide it (ADR-0047). Reconciliation imports an ADR's declared status faithfully, but records no decider — and because deciding is guarded on `proposed`, such a row otherwise asserts a decision that can never be attributed. Not an undo: a design carrying a `decided_by` is refused, as is one whose promotion has already materialized work. |
| `attribute_design_decision` | Record who made a decision the ledger already asserts but attributes to nobody (ADR-0051). Not a decision — status, reason, and promotion state are untouched. Takes exactly the rows `reopen_undecided_design` refuses, whose promotion has materialized work, so the two verbs partition the undecided rows instead of overlapping. A `decided_by` already present is never overwritten. |
| `supersede_design` | Record that an accepted design has been replaced by another (ADR-0050). Not retirement (the record should not have existed) and not rejection (the decision was refused): superseding says the decision was made, it held, and something better replaced it — so the row stays `accepted` and gains a link to its successor, mirroring a goal's `superseded_by`. Only a design with a recorded decider can be superseded, the replacement must already be registered, and the link is never inferred from an ADR's `Superseded by` prose. |
