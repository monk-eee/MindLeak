# ADR-0082: Backplane is a standalone federation service
...
credible boundary for Mission Control across many repositories, machines, and
teams.
...
1. **MindLeak Backplane is a separately deployable service.** It is not a mode
   of `mindleak-mcp`, `lodestar-mcp`, the VS Code extension, or Mission Control.
   Those remain clients or repository nodes. A Backplane deployment owns an
   organisation boundary and may contain many repository identities.

2. **Backplane federates domain records; it does not mount or replicate local
   ...
   contract. Mission Control reads Backplane projections and never queries a
...
3. **Authority is partitioned, never dual-written.** A repository has one
  ...
  mode, Backplane is the sole arbiter for the enrolled repository's shared task
  ...
  Backplane records their exported receipts; it does not silently recompute or
  overwrite them.

4. **Policy distribution cannot become policy activation.** Backplane may
  ...
  flow. Backplane has no shorter activation path. An organisation administrator
  cannot turn a proposal into active repository law merely by changing a
  Backplane setting.
...
5. **Repository connections are outbound and interruption-tolerant.** A node
   ...
   the last acknowledged record. Backplane assigns an ingestion position and
...
6. **Disconnection is visible state, not silent success.** Local memory,
   ingestion, advice, and repository conformance continue when Backplane is
   unavailable. An existing federated claim remains authoritative only until
   its fixed Backplane-recorded lease expiry; a disconnected node cannot renew
  ...
  taking or merging automatically. Mission Control reports the last accepted
  position, age, lease horizon, and uncoordinated records for every repository.

7. **Backplane stores an append-only organisation ledger and derived read
...
8. **Mission Control is an unprivileged Backplane client.** The server may host
...
9. **Backplane v1 aggregates repositories but does not coordinate across their
...
- Backplane can aggregate exact receipts without pretending every uploaded
