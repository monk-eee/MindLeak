...
repository node. Generating the private key on Backplane and downloading it
...
1. **Enrolment is an explicit state machine.** A node moves through
   ...
   Backplane is the authority for that state and appends every transition with
...
3. **A pending request names the exact identity being requested.** It contains
   a random request id, tenant id, explicit Backplane repository id, proposed
...
4. **An authenticated administrator approves the fingerprint, not a label.** A
   ...
   pending request through Backplane. Approval records the exact fingerprint,
...
5. **Activation requires fresh proof of possession.** After approval,
   Backplane issues a single-use, short-lived nonce bound to the request id,
   tenant, repository, node, approved fingerprint, and server time. The node
   signs the domain-separated challenge. Backplane verifies the signature,
...
6. **Activation is confirmed remotely and persisted locally.** The node stores
   the Backplane endpoint, tenant id, repository id, node id, key id, enrolment
   ...
   Backplane appends the `active` transition. Until then it stays `activating`,
...
7. **Rotation proves continuity with both keys.** An active node generates the
   ...
   its current key. Backplane appends the new binding and permits a bounded
...
9. **Losing the key loses the node identity.** A fresh container or machine may
   ...
   approval. Backplane never recovers, exports, or regenerates the lost private
...
11. **Expired and rejected requests leave no authority behind.** They remain in
    ...
    request and key challenge; Backplane does not extend an expired bootstrap
...
- Backplane needs a certificate issuer, key and certificate expiry monitoring,
...
**Put a static Backplane API key in an environment variable.** Rejected because
...
**Generate and escrow node private keys on Backplane.** Rejected because a
