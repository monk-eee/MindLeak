...
### The accept→decompose bridge

On `accept_design`, transactionally:

1. **Decompose** the accepted ADR into one or more implementation tasks under the
   appropriate objective goal — generalize `decompose_goal` from a goal to an
   accepted design item (this is what solves large-ADR breakdown). Tasks are
   created **claimable** (or as a linear `blocked_by` chain only when they edit
   the same file — a code-to-code handoff, never behind the design item).
2. **Register** any durable constraints/invariants the ADR mandates into the
   Constitution via the normal `define_goal` path, so conformance enforces them.
3. **Link** the spawned tasks and goals back to the ADR id for provenance rollup.
...
actions, and shows the downstream decomposed tasks once accepted. The Intent Board
is improved to (a) exclude tainted design items, and (b) show which executive
tasks descend from which accepted ADR (provenance rollup). Keep vscode-coupled
code thin; pure board/threading logic in `editors/vscode/src/util.ts` with vitest.
...
- `decompose_goal` is generalized; new surface in `lodestar-core` (design-item
  store + `accept_design`/`reject_design`) and `lodestar-mcp` (tool defs), plus
  the Design Board portal view. New behaviour gets tests: a tainted item is
  invisible to `next_task`/executive board; accept decomposes into ≥1 claimable
  task and registers stated constraints; reject leaves an audit record and spawns
  no work; the whole path works with no local model.
