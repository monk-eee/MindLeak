...
describe("canonical-push", () => {
  ...
  it("provides both MCP planes for claimed publication", () => {
    const root = mkdtempSync(join(tmpdir(), "mindleak-claim-environment-"));
    temporaryDirectories.push(root);
    const environment = claimEnv(root, { claims: [liveClaim] });

    // Copying Git repositories made this environment-only check exceed the
    // default CI timeout. Its fixture needs only the two planes' ledger stub.
    expect(readdirSync(root)).toEqual(["stub-ledger.mjs"]);
