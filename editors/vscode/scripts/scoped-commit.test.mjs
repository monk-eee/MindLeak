...
describe("scoped-commit", () => {
  ...
  it("commits a staged rename whose old path isn't declared, without resurrecting its content", () => {
    ...
    // Enough shared content for git's default -M50 rename heuristic to pair
    // the old and new paths as a rename rather than an unrelated delete+add.
    const original = Array.from({ length: 20 }, (_, i) => `line ${i}\n`).join("");
    writeFileSync(join(repo, "module.rs"), original);
    git(repo, ["add", "module.rs"]);
    git(repo, ["commit", "-m", "base"]);

    mkdirSync(join(repo, "module"));
    git(repo, ["mv", "module.rs", "module/mod.rs"]);
    writeFileSync(join(repo, "module", "mod.rs"), `${original}// one more line\n`);

    const result = spawnSync(
      process.execPath,
      [scopedCommit, "-m", "scoped", "--", "module/mod.rs"],
      { cwd: repo, encoding: "utf8", env: isolatedGitEnvironment() }
    );

    expect(result.status, `${result.stdout}\n${result.stderr}`).toBe(0);
    expect(git(repo, ["show", "--name-status", "--format=", "HEAD"])).toContain("module/mod.rs");
    const oldContent = spawnSync("git", ["cat-file", "-p", "HEAD:module.rs"], {
      cwd: repo,
      encoding: "utf8",
      env: isolatedGitEnvironment(),
    });
  ...
  it("does not commit an undeclared rename's old path when the new path is also undeclared", () => {
    const repo = mkdtempSync(join(tmpdir(), "mindleak-scoped-commit-rename-"));
    temporaryDirectories.push(repo);
    git(repo, ["init", "-b", "main"]);
    git(repo, ["config", "user.name", "Scoped Commit Rename Test"]);
    ...
    const original = Array.from({ length: 20 }, (_, i) => `line ${i}\n`).join("");
    writeFileSync(join(repo, "unrelated.rs"), original);
    git(repo, ["add", "unrelated.rs"]);
    git(repo, ["commit", "-m", "base"]);

    // Another agent's in-flight rename, unrelated to this commit's own path.
    mkdirSync(join(repo, "unrelated"));
    git(repo, ["mv", "unrelated.rs", "unrelated/mod.rs"]);
    writeFileSync(join(repo, "mine.txt"), "declared\n");
    ...
    expect(result.status, `${result.stdout}\n${result.stderr}`).toBe(0);
    expect(git(repo, ["show", "--name-only", "--format=", "HEAD"])).toBe("mine.txt");
    // The other agent's staged rename is untouched -- still staged, not committed.
    expect(git(repo, ["diff", "--cached", "--name-status"])).toContain("unrelated/mod.rs");
    expect(result.stderr).toMatch(/staged paths are not yours/);
  }, 30_000);
