- **A packaged extension binary outranks every other install, so a stale VSIX silently serves stale servers.** Observed while fixing
  ...
  launches, and nothing reports the mismatch; the packaged copy has no version
  ...
  higher. Worth either recording the source commit alongside a staged binary and
  warning when it predates the extension's own, or having the extension report
  which of the three candidate paths it resolved. Left for later: only the
  immediate deployment was corrected this run.
