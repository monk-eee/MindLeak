...
.PHONY: setup worktree-setup install-servers install-industrial package-industrial adr-index changelog design-audit merge-audit binding-audit queue queue-watch sweep board-health control-coverage stranded-report migration-audit status reingest tool-surface build test industrial-test script-test ratchet coverage bench agent-bench lint fmt fmt-check clippy run ext-install ext-compile ext-lint ext-test ci
...
package-industrial: ## Build a revision-bound Industrial host archive from a clean checkout
	node scripts/industrial-bundle.mjs $(ARGS)
...
binding-audit: ## Report goal/code binding defects; ARGS=--check to fail on them, --repair to fix a split
	node scripts/binding-audit.mjs $(ARGS)
...
control-coverage: ## Report normative clauses with no active control, or one weaker than declared (ADR-0034)
	node scripts/control-coverage.mjs
