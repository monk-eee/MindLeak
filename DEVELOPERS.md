...
| Board health       | `make board-health`    | `node scripts/board-health.mjs` — separates parked work a human must decide from work nobody can resolve, lists stranded claims (ADR-0058), and reports orphaned `gaps.d/` fragments (no tracking task at all). Needs `LODESTAR_SESSION_ID` and a release `lodestar-mcp`                                                                                                                                                                                                                                                                                                                                                              |
...
| Migration audit    | `make migration-audit` | `node scripts/migration-audit.mjs` — reports a duplicate migration key, a committed constant/file with no match, a migration this branch edited that already exists on `origin/main`, a key a live database applied that committed source never declared, or a key applied under content the committed migration no longer carries. Audits both `ackplane` and `ackplane_test`. Select Podman with `--engine podman` (or `make migration-audit ARGS="--engine podman"`); engine precedence is `--engine`, then `MINDLEAK_COMPOSE_BIN`, then `docker`. The engine is one executable, not a shell command; use `--container <name>` to select the database container. `--check` gates on the static findings and runs in CI. `--next` (or `ARGS=--next`) prints the safe next key, folding in whatever the shared dev Postgres already has applied so a concurrent branch's not-yet-committed number cannot collide with it. This only detects a collision after the fact; `ackplane-migrate --mark-shared` (run once against a shared or persistent Postgres instance, never an ephemeral local/CI container) closes the other half — every later migration attempt against a marked database refuses unless `ACKPLANE_MIGRATE_REVIEWED=1` is set, which should only happen after confirming the migration's key via this same audit |
...
they can be declared with `also_serves` _before_ the work rather than discovered
