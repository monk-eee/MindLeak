...
| Migration audit    | `make migration-audit` | `node scripts/migration-audit.mjs` — reports a duplicate migration key, a committed constant/file with no match, a key a live database applied that committed source never declared, or a key applied under content the committed migration no longer carries. Audits both `ackplane` and `ackplane_test`. `--next` (or `ARGS=--next`) prints the safe next key, folding in whatever the shared dev Postgres already has applied so a concurrent branch's not-yet-committed number cannot collide with it |
